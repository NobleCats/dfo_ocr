...
content = """# api/routes/realtime.py
...
def _trigger_background_update(server: str, name: str):
    ...
    try:
        # Get the global event loop (started by Flask/app.py)
...
def _build_fast_response_from_row(row: dict) -> dict:
    ...
    Uses cached score columns only. Does not calculate rank, and does not call
    any external calculator/API.
    \"\"\"
    is_buffer = bool(row.get(\"is_buffer\"))
...
target_path = r'C:\MesugakiProto\dfogang_backend\api\routes\realtime.py'
